...
Make your answers concise and professional.
"""
...
def build_conversation_prompt(messages, include_persona=False):
    ...
    # Provide design instructions always; let AI decide how to respond based on user input
    design_instructions = """
If the user's query is about design or sizing a solar inverter system, please respond with:
1. Inverter size (kVA)
2. Battery size (kWh)
3. Solar panel size (Watt-peak)
4. Explanation for your choice in a single paragraph.

<Example>
1. Inverter size (kVA): 3.5kVA
2. Battery size (kWh): 4kWh
3. Solar panel size (kW-peak): 1kW
